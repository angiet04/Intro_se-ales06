...
## Discusión
### ECG
A partir del análisis realizado y la experimentación con la transformada wavelet para la eliminación de ruido en señales ECG, se puede afirmar que el uso de esta técnica es sumamente efectivo. Se utilizaron diversas funciones wavelet, como 'sym8' y 'coif2', y se aplicaron reglas de umbralización como 'rigrsure', siguiendo un enfoque de descomposición multinivel y reconstrucción de la señal.
Los resultados obtenidos corroboran la literatura existente, demostrando que la adecuada selección de la función wavelet y la aplicación de umbralización suave mejoran significativamente la claridad de la señal al minimizar el ruido presente, mientras se preservan los elementos críticos del patrón del ECG. Esto se evidencia especialmente en la capacidad de las funciones seleccionadas para aislar y reducir componentes de ruido sin comprometer la integridad de los datos cardiacos fundamentales.
La transformada wavelet mediante una selección cuidadosa de funciones y reglas de umbral, ofrece una herramienta eficiente para el procesamiento y mejora de señales ECG.
### EMG
